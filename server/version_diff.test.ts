...
const TMP_DB = './tmp_version_diff.db';
...
describe('Post Diff', () => {
  ...
  it('should compute a diff between latest version and head', async () => {
    const postId = await db.createPost({ userId: 1, title: 'Title', content: 'Hello', contentType: 'plaintext', published: 1 } as any);
    // update to create a version
    await db.updatePost(postId, { title: 'Title', content: 'Hello World' });
    const versions = await db.getPostVersions(postId);
    expect(versions.length).toBeGreaterThan(0);
    // use routers function via db-like diff logic
    // We'll directly compare strings: old vs new
    const base = versions[0].content;
    const head = (await db.getPostById(postId))!.content;
    const res = require('diff').createPatch(`post-${postId}`, base, head);
